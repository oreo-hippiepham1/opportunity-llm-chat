...
st.set_page_config(page_title="Job App made easy", page_icon="🐳", layout="wide")
...
class ResumeAssistant:
        ...
        if uploads["cv"] and not st.session_state["cv_data"]:
            with st.spinner("Parsing the PDF..."):
                try:
                    ...
                    st.session_state["cv_data"] = extract_full_text_from_chunks(
                        cv_chunks
                    )
        ...
        if uploads["letter"] and not st.session_state["letter_data"]:
            with st.spinner("Parsing the DOCX..."):
                try:
                    ...
                    # Convert the chunks into 1 full string, and stores in session_state
                    st.session_state["letter_data"] = extract_full_text_from_chunks(
                        letter_chunks
                    )
                except Exception as e:
        ...
        if analyze_button:
            if st.session_state.get("cv_data") and st.session_state.get("letter_data"):
                with st.spinner("Analysis in progress..."):
                    response = generate_pre_analysis(
    ...
    def opp_process(self):
        ...
        opp_desc_type = st.selectbox(
            label="How do you want to upload opportunity desc?",
            options=["Text", "PDF", "DOCX"],
            index=0,
        )

        if opp_desc_type == "Text":
            opp_desc = st.text_area(label="Paste the opportunity here...", height=250)

            if opp_desc:
                st.session_state["opp_desc_data"] = extract_full_text_from_chunks(
                    [opp_desc]
                )
            ...
            opp_desc_upload = st.file_uploader(
                "Opportunity Flyer (DOCX)", type=["docx"]
            )
            if opp_desc_upload:
                try:
    ...
    def add_reqs(self):
        ...
        additional_req = st.text_area(
            "Any additional requirements you want to add for the cover letter / cold email...",
            height=200,
        )
        if additional_req:
    ...
    def final_gen(self):
        ...
        if (
            "cover_generated" in st.session_state
            and st.session_state["cover_generated"]
        ):
    ...
    def main(self):
        ...
        st.markdown(
            "**[Optional] Generate a pre-analysis of your profile's strength and weaknesses.**\n\n"
        )
        self.pre_analysis()

        st.header("3. Opportunity Uploads")
        st.markdown(
            "**Paste, or upload that job / scholarship etc.. description.**\n\n"
        )
        self.opp_process()

        st.header("4. Generation! 📝")
        st.markdown(
            "**Write any additional requests you would like to have (e.g. Draft a cold emailing focusing on ....) and Generate!**\n\n"
        )
        self.final_gen()
